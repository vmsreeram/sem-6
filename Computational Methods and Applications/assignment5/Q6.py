...
def demonOrthoChebyshev():
    ...
    C = []
    for i in range(DEMO_UPTO):
        C.append(nthChebyshev(i))
    
    # we demonstrate by displaying the matrix and verifying if it is identity matrix or not
    for ii in range(DEMO_UPTO):
        for jj in range(DEMO_UPTO):
            def integ_fn(x):
                return wt_fn(x)*C[ii][x]*C[jj][x]
            print('%.2f'%quad(integ_fn,L_BOUND,U_BOUND)[0], end='\t')   # we truncate and round to 2 places and print
        print('\n')
