...
def FindRoots_Aberth_modified(fun, a, b, degree, maxIter=100):
    EPS = 0.00001 # define a small number to represent the desired accuracy of roots
    
    points=[]
    for i in np.arange(a,b,0.1):
        points.append([i,fun(i)]) # create a list of (x,y) points of the function

    poly = bestFitPoly(points=points,n=degree) # fit a polynomial of degree n to the points
    poly_deriv = poly.derivative() # compute the derivative of the polynomial
    
    X = list(np.linspace(a,b,degree)) # initialize a list of equally spaced points between a and b as initial guesses for roots
    N = len(X)
        
    # start the Aberth method to compute the roots of the polynomial
    for iter in range(maxIter):
        X.sort()  # sort the current computed roots in ascending order to compare with the actual roots
        flag = True

        # check if the absolute value of the polynomial at the computed roots is less than EPS, and if the computed roots are within [a-EPS,b+EPS]
        for i in range(N):
            if(abs(poly[X[i]])>EPS and X[i] > a-EPS and X[i]<b+EPS):
                flag = False
                break
	
        # if the computed roots are within EPS of the actual roots, return the roots
        if(flag == True):
            ans=[]
            for i in range(N):
                if(X[i] > a-EPS and X[i]<b+EPS):
                    ans.append(X[i])
            return ans
        ...
        for i in range(N):
            ...
            # if the derivative is zero, set it to zero to avoid division by zero
            thisder = poly_deriv[X[i]] if(poly_deriv[X[i]] != 0) else 0

            numr = poly[X[i]] / thisder  # compute the numerator
            denr = 1 - numr * dsum  # compute the denominator

            # add the new computed root to the list
            Xnext.append(X[i] - numr/denr)
    ...
    # if the maximum number of iterations is reached, return the computed roots
...
def fun(x):
    return np.exp(x)-4
roots = FindRoots_Aberth_modified(fun,-1,4,20,maxIter=1000) # compute the roots of the function over [-1,4] with a degree of 20
for root in roots:
    print("fun(",root,") =", fun(root)) # print the roots and their corresponding values of the function