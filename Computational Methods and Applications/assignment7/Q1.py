...
def initial_condition(x):
    c = [0 if (i==0 or i==L) else 1 for i in x]
    return np.exp(-x)*(c)
...
def heat_conduction_pde(t, u):
    ...
    dudt[1:-1] = alpha * (u[2:] - 2 * u[1:-1] + u[:-2]) / \
        ((L / (len(u) - 1)) ** 2)
    dudt[0] = 0
    dudt[-1] = 0
    return dudt
...
sol = solve_ivp(heat_conduction_pde, t_span, initial_condition(
    x), t_eval=np.linspace(t_span[0], t_span[1], 200))
